...
export default class ServerStats {
  ...
  private getOnlineMembers() : void {
    ...
    const membersCount = this.guild.members.cache.filter((member : GuildMember) => this.isMemberOnline(member)
  ...
  private getMembersByRank(channelName : string, roleName : string, channelNameWithCounts : ServerStatsEnum) {
    ...
    const count = this.guild.members.cache.filter((member : GuildMember) => ServerStats.isNotABot(member)
